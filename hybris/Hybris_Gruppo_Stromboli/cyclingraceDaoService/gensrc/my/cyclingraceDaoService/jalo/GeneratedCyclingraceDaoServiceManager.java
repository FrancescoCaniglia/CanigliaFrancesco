...
public abstract class GeneratedCyclingraceDaoServiceManager extends Extension
{
	...
	public GenericItem createTeam(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( CyclingraceDaoServiceConstants.TC.TEAM );
		...
		{
			throw new JaloSystemException( e ,"error creating Team : "+e.getMessage(), 0 );
		}
	}
	
	public GenericItem createTeam(final Map attributeValues)
	{
		return createTeam( getSession().getSessionContext(), attributeValues );
	}
